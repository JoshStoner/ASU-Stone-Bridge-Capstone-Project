...
class InspectionFormViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate //, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    @IBAction func publishForm(_ sender: Any) {
        ...
        //  InspectionFormEntity then into the InspectionFormCategoryEntity then into the InspectionFormSectionEntity then into the InspectionFormPicContainerEntity and finally the InspectionFormPicturesEntity in order to get everything inside the categories
        
        if (load == false)
        {
            var InspectionFormData = iModel?.searchEnt(sWellName: wellNameField.text!, sDate: dateField.text!, sWellNumber: Int(wellNumberField.text!)!, sInspectionDone: inspectionDoneField.text!)
...
//                print(InspectionFormData?.inspectionDone as Any)
                ...
                var sortedSections = (sortSections?.allObjects as! [InspectionFormCategoryEntity]).sorted(by: {$0.tagN < $1.tagN})
                var i = 0;
